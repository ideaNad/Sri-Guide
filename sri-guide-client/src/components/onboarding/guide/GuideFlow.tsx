      ...
      <div className="space-y-6">
        <div className="flex flex-col items-center gap-6 p-8 border-2 border-dashed border-slate-200 rounded-3xl bg-white hover:border-sky-300 transition-colors cursor-pointer group">
          <div className="p-4 bg-slate-50 rounded-full group-hover:bg-sky-50 transition-colors">
            <Camera className="w-12 h-12 text-slate-400 group-hover:text-sky-600" />
          </div>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <input 
            type="text" 
            placeholder="Trip Title (e.g. Ella Nine Arches Sunset)" 
            value={data.firstTripTitle || ''}
            onChange={(e) => setData({ firstTripTitle: e.target.value })}
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-grow">
          ...
          <input 
            ...
            onChange={(e) => setData({ listingPrice: e.target.value })}
            className="w-full p-4 pl-12 rounded-xl border border-slate-200 text-2xl font-black" 
          />
        </div>
        <select 
          value={data.priceUnit || 'per Hour'}
          onChange={(e) => setData({ priceUnit: e.target.value })}
          className="p-4 rounded-xl border border-slate-200 font-bold bg-white"
        >
