...
import {
    LayoutDashboard, Map, Users, Star, User,
    Calendar, Plus, LogOut, Menu, X, HelpCircle, 
...
const ORGANIZER_NAV = [
    ...
    { name: "My Events", href: "/organizer/events", icon: <Calendar size={20} /> },
    { name: "Create Event", href: "/organizer/events/new", icon: <Plus size={20} /> },
    { name: "My Reviews", href: "/organizer/reviews", icon: <Star size={20} /> },
    { name: "My Profile", href: "/organizer/profile", icon: <User size={20} /> },
];
...
export default function OrganizerLayout({ children }: { children: React.ReactNode }) {
    ...
    if (loading || !user || user.role !== "EventOrganizer") {
        return (
    ...
    const SidebarContent = ({ pathname, setSidebarOpen, logout, user }: { pathname: string, setSidebarOpen: (open: boolean) => void, logout: () => void, user: any }) => {
        return (
            <div className="flex flex-col h-full bg-white border-r border-gray-100 relative overflow-y-auto overflow-x-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-orange-50/50 blur-3xl rounded-full -mr-16 -mt-16 -z-10" />
                ...
                <div className="px-6 pb-6">
                    <div className="flex items-center gap-3 p-3 bg-orange-50/50 rounded-2xl border border-orange-100/50 shadow-sm transition-all hover:bg-orange-50">
                        <div className="w-10 h-10 rounded-full bg-orange-500/10 flex items-center justify-center text-orange-600 font-bold border border-orange-500/20 overflow-hidden shrink-0 shadow-inner">
                            {user?.profileImageUrl ? (
                                ...
                                <span>{user?.fullName?.charAt(0) || "O"}</span>
                            )}
                        </div>
                        <div className="overflow-hidden">
                            <p className="text-sm font-black text-gray-900 truncate leading-tight uppercase tracking-tight">{user?.fullName || "Organizer"}</p>
                            <p className="text-[10px] font-black text-orange-600 uppercase tracking-widest leading-none mt-1">Event Organizer</p>
                        </div>
                    </div>
                </div>

                <nav className="flex-1 px-4 space-y-1">
                    <p className="text-[10px] font-black text-orange-600 uppercase tracking-[0.3em] px-4 mb-4">Main Navigation</p>
                            ...
                            <Link
                                ...
                                className={`flex items-center justify-between px-6 py-4 rounded-2xl transition-all duration-300 group ${
                                    isActive
                                        ...
                                        : "text-gray-500 hover:bg-orange-50 hover:text-orange-700 font-bold"
                                }`}
                            >
                                <div className="flex items-center gap-4">
                                    <span className={isActive ? "text-orange-400" : "text-gray-400 group-hover:text-orange-600 transition-colors"}>
                                        {item.icon}
                                    </span>
                                    <span className="text-sm tracking-wide">{item.name}</span>
                                </div>
                                {isActive && <motion.div layoutId="activeInd" className="w-1.5 h-1.5 rounded-full bg-orange-400" />}
                            </Link>
                        );
                    })}
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-between px-6 py-4 rounded-2xl text-gray-500 hover:bg-orange-50 hover:text-orange-700 transition-all duration-300 group font-bold mt-4"
                    >
                        <div className="flex items-center gap-4">
