...
export default function AgencyTripsPage() {
    ...
    const handleCreateTrip = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingTripId) {
                await apiClient.put(`/Agency/trips/${editingTripId}`, { ...newTrip, tripId: editingTripId });
            } else {
                const response = await apiClient.post("/Agency/trips", newTrip);
            ...
            if (selectedFiles.length > 0 && tripId) {
                for (const file of selectedFiles) {
                    ...
                    await apiClient.post(`/Agency/trips/${tripId}/upload-photo`, formData, {
    ...
    const handleRemoveExistingImage = async (imageUrl: string) => {
        ...
        try {
            ...
            fetchTrips(); // Refresh the overarching list to get the updated imageUrl if needed
        } catch (error) {
            ...
            <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div>
                    ...
                    <h1 className="text-3xl sm:text-4xl font-black text-gray-900 tracking-tighter uppercase italic">
                        Past <span className="text-teal-600 text-stroke-primary">Experiences</span>
                    </h1>
                ...
                {!isAdding && (
                    ...
                    >
                        <Plus size={18} /> Add Past Experience
                    </button>
                )}
            </header>
                                            ...
                                            {existingImages.map((imgUrl, i) => (
                                                <div key={`existing-${i}`} className="h-20 w-20 rounded-xl overflow-hidden shadow-sm relative group z-10">
                                                    ...
                                                    <button 
                                                        ...
                                                        className="absolute inset-0 bg-black/10 hover:bg-black/40 flex items-center justify-center text-white transition-all shadow-lg"
                                                    >
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                            ...
                            trips.map((trip, i) => (
                                ...
                                >
                                    <div className="h-48 bg-gray-100 relative group/image">
                                        {trip.imageUrl ? (
                                            <img 
                                                src={trip.imageUrl?.startsWith("/") ? `${apiClient.defaults.baseURL?.replace('/api', '')}${trip.imageUrl}` : trip.imageUrl} 
                                                alt={trip.title} 
                                                className="w-full h-full object-cover" 
                                            />
                                        ) : (
                                        ...
                                        <div className="absolute inset-0 bg-black/10 group-hover:bg-black/40 transition-all flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
