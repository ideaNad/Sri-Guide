...
interface ReviewDto {
    ...
    comment: string | null;
    createdAt: string;
    targetType: "Guide" | "Trip" | "Agency" | "Tour";
...
export default function ReviewsClient({ title, type }: ReviewsClientProps) {
    const [reviews, setReviews] = useState<ReviewDto[]>([]);
    const [loading, setLoading] = useState(true);
    const [filter, setFilter] = useState<"All" | "Profile" | "Item">("All");

    useEffect(() => {
        const fetchReviews = async () => {
            try {
                const response = await apiClient.get<ReviewDto[]>(`/review/guide-reviews?type=${type}`);
    ...
    const filteredReviews = reviews.filter(r => {
        ...
        if (filter === "Profile") return r.targetType === "Agency" || r.targetType === "Guide";
        if (filter === "Item") return r.targetType === "Tour" || r.targetType === "Trip";
        return true;
    });
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-gray-900 tracking-tighter uppercase italic">
                        {title.split(' ')[0]} <span className="text-teal-600">{title.split(' ').slice(1).join(' ')}</span>
                    </h1>
                    <p className="text-gray-500 font-bold mt-2">See what your guests are saying about your {type === 'agency' ? 'agency and tours' : 'guide profile'}.</p>
                </div>
            </header>
                    ...
                    <div className="bg-gray-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden">
                        <div className="relative z-10 text-center">
                            ...
                            <div className="flex items-center justify-center gap-3 mb-2">
                                ...
                                <Star size={48} className="text-teal-400 fill-teal-400 drop-shadow-[0_0_15px_rgba(45,212,191,0.5)]" />
                            </div>
                            <p className="text-xs font-bold text-white/50 uppercase tracking-widest">
                                Based on {reviews.length} total reviews
                            </p>
                        </div>
                        <div className="absolute top-0 right-0 w-64 h-64 bg-teal-400/10 rounded-full blur-[80px] -mr-32 -mt-32" />
                    </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-2xl w-fit">
                        {["All", "Profile", "Item"].map(f => (
                            ...
                            >
                                {f === "All" ? "All Reviews" : f === "Profile" ? "Profile" : (type === 'agency' ? "Tour/Trip" : "Trip")}
                            </button>
                        ))}
                    </div>
                                ...
                                >
                                                ...
                                                <div className="flex items-center gap-2 text-[10px] font-bold text-gray-400 mt-1 uppercase tracking-widest">
                                                    ...
                                                    {review.tripTitle && (
                                                        <>
                                                            ...
                                                            <span className="text-teal-600 px-2 py-0.5 bg-teal-50 rounded-full">
                                                                {review.tripTitle.length > 25 ? review.tripTitle.substring(0,25) + "..." : review.tripTitle}
                                                            </span>
                                                        </>
                                                    )}
                                                    {(review.targetType === "Agency" || review.targetType === "Guide") && (
                                                        <>
                                                            <span className="mx-1">•</span>
                                    ...
                                    {review.comment && (
                                        <div className="mt-4 bg-gray-50/50 p-4 rounded-2xl relative group">
                                            <Quote size={24} className="absolute top-2 left-2 text-gray-200/50 -z-10 group-hover:text-teal-500/10 transition-colors" />
