...
import { Star, MessageCircle, Clock, Car, User, Quote, Truck } from "lucide-react";
...
export default function TransportReviewsPage() {
    ...
    const [filter, setFilter] = useState<"All" | "Transport" | "Vehicle">("All");

    useEffect(() => {
        const fetchReviews = async () => {
            try {
                const response = await apiClient.get<ReviewDto[]>("/review/transport-reviews");
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-gray-900 tracking-tighter uppercase italic">
                        Fleet & Profile <span className="text-blue-600">Reviews</span>
                    </h1>
                    <p className="text-gray-500 font-bold mt-2">Manage feedback for your vehicles and service.</p>
                </div>
            </header>
                    ...
                    <div className="bg-gray-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden">
                        <div className="relative z-10 text-center">
                            ...
                            <div className="flex items-center justify-center gap-3 mb-2">
                                ...
                                <Star size={48} className="text-blue-500 fill-blue-500 drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]" />
                            </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-2xl w-fit">
                        {["All", "Transport", "Vehicle"].map(f => (
                            ...
                            >
                                {f === "All" ? "All Reviews" : f === "Transport" ? "Profile Reviews" : "Vehicle Reviews"}
                            </button>
                        ))}
                    </div>
                                ...
                                >
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 text-[10px] font-bold text-gray-400 mt-1 uppercase tracking-widest">
                                                    ...
                                                    {review.targetType === "Vehicle" && review.vehicleName && (
                                                        <>
                                                            <span className="mx-1">•</span>
                                                            <Car size={10} className="text-blue-600" />
                                                            <span className="text-blue-600 px-2 py-0.5 bg-blue-50 rounded-full">
                                                                {review.vehicleName}
                                                            </span>
                                                        </>
                                                    )}
                                                    {review.targetType === "Transport" && (
                                                        <>
                                                            <span className="mx-1">•</span>
                                    ...
                                    {review.comment && (
                                        <div className="mt-4 bg-gray-50/50 p-4 rounded-2xl relative group">
                                            <Quote size={24} className="absolute top-2 left-2 text-gray-200/50 -z-10 group-hover:text-blue-600/10 transition-colors" />
